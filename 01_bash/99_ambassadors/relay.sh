...
mkdir -p $4 && ffmpeg -hide_banner -i $f -i $a -c:v copy -map 0:0 -map 1:0 -c:a copy $4/$basef--$basea--$date.mov
ffmpeg -hide_banner -y -i "$4/$basef--$basea--$date.mov" -c:v libx264 -c:a aac -b:v "$3"k -b:a 256k -profile:v high -level 41 -pix_fmt yuv420p -f mp4 "$4/$basef--$basea--$date-$time.mp4"
