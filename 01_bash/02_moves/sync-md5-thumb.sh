...
# run the script if there are more than two arguments provided
elif [[ "$#" -ge 2 ]]; then
...
# set the bit that will decide if thumbnails are created later on
...
## and make an md5 hash
  ...
  out1File=$(find "$2" -type f -and -iname "$inFileBase" -and -ipath "*/$inFileSearchFolder/*" -and -not -iname ".*")
  md5Out1=$(md5 -q "$out1File")
  out1FileBase=$(basename $out1File)

  echo "Checking file integrity for"
  echo $out1FileBase
  ...
  if [[ -n "$3" ]] || [[ -d "$3" ]]; then
    ...
    out2File=$(find "$3" -type f -and -iname "$inFileBase" -and -ipath "*/$inFileSearchFolder/" -and -not -iname ".*")
  ...
  if [[ $thumbselect == "1" ]]; then
    ...
    cp -f "$2/thumbnails/$inFileBlank.png" "$3/thumbnails/$inFileBlank.png"
